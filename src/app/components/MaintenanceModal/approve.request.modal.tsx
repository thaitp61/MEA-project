...
export default function ApproveModal(props: IProps) {
    ...
    const handleApproveRequestRepair = async () => {
        if (!note) {
            toast.error("Không để trống ghi chú")
            return;
        }
        try {
            const response = await ApiContext.put(`/repair-request/${repairRequestID}/approve`, {
                note: note,
            });
    ...
    return (
        <Dialog
            ...
            maxWidth='sm'
        >
            <DialogTitle className="title-text" id="scroll-dialog-title">XÁC NHẬN YÊU CẦU</DialogTitle>
            ...
            <DialogActions>
                ...
                <Button
                    ...
                    onClick={handleApproveRequestRepair}
                >Xác nhận</Button>
            </DialogActions>
        </Dialog>
